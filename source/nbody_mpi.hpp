...
inline void nbody_mpi(
	int MPI_rank, int MPI_size, MPI_Datatype MPI_VEC3, MPI_Datatype MPI_BODY_PART,
	...
	if (!benchmark_mode) {
		...
		for (size_t i = 0; i < block_size; ++i) {
			const int number_width = 1 + static_cast<int>(log10(block_start_index + i + 1));

			filenames[i] =
				output_folder + "/positions_" +
				std::string(max_number_width - number_width, '0') + std::to_string(block_start_index + i + 1) + ".txt";
		}
	...
	for (size_t step = 0; step < iterations; step++) {
		...
		MPI_Allgather(
			MPI_IN_PLACE, 0, MPI_DATATYPE_NULL,
			bodies.data(), block_size, MPI_BODY_PART,
			MPI_COMM_WORLD
		);

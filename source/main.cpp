...
const size_t TIME_LAYER_EXPORT_STEP = 1;
...
const T m_factor = 1; //1;
const T r_factor = 1; //149597870.700e3; 
const T v_factor = 1; //1731460;

bool BENCHMARK_MODE =         true;
bool TEST_CONVERGENCE_ORDER = false;
bool USE_RANDOM_BODIES =      true;

bool SKIP_SERIAL_METHOD = true;

// # Random config #
const size_t RANDOM_N = 1024 * 18 * 5;
...
int main(int argc, char* argv[]) {
	...
	std::string msg("Rank " + std::to_string(MPI_rank) + "/" + std::to_string(MPI_size) + " initialized\n");
	...
	if (MPI_rank == 0) {
		...
		for (size_t i = 0; i < bodies.size(); ++i) {
			bodies[i].mass *= m_factor;
			bodies[i].position *= r_factor;
			bodies[i].velocity *= v_factor;
		}
	...
	// --- 1) Serial method ---
	// ------------------------
	if (!SKIP_SERIAL_METHOD && MPI_rank == 0) {
		// 1. Method
		table_add_1("Serial");
	...
	msg = std::string("Rank " + std::to_string(MPI_rank) + "/" + std::to_string(MPI_size) + "  finalized\n");
	std::cout << msg << std::flush; 
